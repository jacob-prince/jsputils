...
def get_model_selective_units(model_name, floc_imageset, input_model_and_transforms = None,
    ...
    if floc_imageset == 'vpnl-floc':
        floc_domains = ['faces','bodies','objects','scenes','characters']
    elif floc_imageset == 'classic-categ':
        floc_domains = ['1-Faces','2-Bodies','3-Scenes','4-Words','5-Objects']
        
    if input_model_and_transforms is not None: 
        model, transforms = input_model_and_transforms
    else:
    ...
    for target_domain in floc_domains:
    
        savefn = f"{paths.selective_unit_dir()}/{model_name}_{floc_imageset}-{target_domain}_FDR-{str(FDR_p)[2:]}.npy"
    ...
    all_selective_units['floc_imageset'] = floc_imageset
    all_selective_units['floc_domains'] = floc_domains
    all_selective_units['model_name'] = model_name
...
def compute_selectivity_old(Y, all_domain_idx, target_domain_val, FDR_p, verbose=True):
